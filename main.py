...
# emoji glyphs are rendered by a different font/engine than plain text,
# so mixing them inside one string (icon + text) never lines up perfectly.
# Putting the icon in its own fixed-width label and the text in its own
# label, both placed on a grid, guarantees they sit on the same baseline
# and stay aligned no matter which emoji is used.
...
class SidebarButton(ctk.CTkFrame):
    ...
    def __init__(self, master, icon, text, command, **kwargs):
        ...
        # Fixed-width icon column keeps every icon's left edge identical,
        # regardless of how wide that particular emoji glyph is.
...
class ClickableRow(ctk.CTkFrame):
    ...
    def __init__(self, master, icon, title, desc, command, is_ai=False, **kwargs):
        ...
        self.configure(fg_color=self.default_color, border_width=1,
                        border_color=COLOR_ACCENT_BORDER if is_ai else COLOR_BORDER)

        self.grid_columnconfigure(0, weight=0, minsize=38)  # icon column, fixed width
        self.grid_columnconfigure(1, weight=1)               # title/desc column
        self.grid_columnconfigure(2, weight=0)                # status column
        self.grid_rowconfigure(0, weight=1)
        self.grid_rowconfigure(1, weight=1)

        title_color = COLOR_ACCENT_TEXT if is_ai else COLOR_TEXT

        # Icon sits in its own fixed-width cell, vertically matched to the title row.
        self.icon_lbl = ctk.CTkLabel(
            self,
            text=icon,
            font=ICON_FONT_ROW,
...
class AndroidAnalyzerApp(ctk.CTk):
    def __init__(self):
        ...
        self.loaded_apk_path = None  # No file is loaded until the user picks one
    ...
    def create_sidebar(self):
        ...
        # Small branding header so the sidebar doesn't start abruptly at "MAIN"
        brand_frame = ctk.CTkFrame(sidebar, fg_color="transparent")
        brand_frame.pack(fill="x", padx=25, pady=(22, 10))
        ctk.CTkLabel(brand_frame, text="🛡️", font=emoji_font(20)).pack(side="left")
        ctk.CTkLabel(brand_frame, text="Android Analyzer", font=("Arial", 15, "bold"),
                     text_color=COLOR_TEXT).pack(side="left", padx=(8, 0))
        ...
        # SCAN MODES Category (Selection only, doesn't change tabs)
    ...
    def create_nav_btn(self, parent, icon, text, page_name):
        """Helper to create main navigation buttons (Changes tabs)"""
    ...
    def create_scan_mode_btn(self, parent, icon, mode_name):
        """Helper to create scan mode selection buttons (Selection only, stays on current view)"""
    ...
    def create_main_content(self):
        ...
        # Result pages corresponding to the middle list buttons
    ...
    def build_dashboard(self):
        ...
        # 1. APK File Header — starts in an empty/"no file" state, not fake sample data
        self.apk_frame = ctk.CTkFrame(page, fg_color=COLOR_SURFACE, corner_radius=10,
                                       border_width=1, border_color=COLOR_BORDER)
        self.apk_frame.pack(fill="x", padx=30, pady=(30, 15))

        self.apk_icon_lbl = ctk.CTkLabel(self.apk_frame, text="📦", font=emoji_font(26),
                                          fg_color=COLOR_DISABLED, text_color="white",
                                          corner_radius=8, width=56, height=56)
        ...
        self.apk_name_lbl = ctk.CTkLabel(info_frame, text="No APK selected", font=("Arial", 16, "bold"),
                                          text_color=COLOR_TEXT)
        self.apk_name_lbl.pack(anchor="w")
        self.apk_path_lbl = ctk.CTkLabel(info_frame, text="Click \"Choose File\" to load an .apk for analysis",
                                          font=("Arial", 12), text_color=COLOR_TEXT_MUTED)
        self.apk_path_lbl.pack(anchor="w")

        self.start_btn = ctk.CTkButton(self.apk_frame, text="▶ Start Analysis", height=40,
                                        fg_color=COLOR_ACCENT, hover_color=COLOR_ACCENT_HOVER,
                                        state="disabled",
                                        command=lambda: print(f"Running scan with mode: {self.current_scan_mode}"))
        self.start_btn.pack(side="right", padx=20)
        ctk.CTkButton(self.apk_frame, text="Choose File", height=40, fg_color=COLOR_SURFACE_HOVER,
                      hover_color="#3D3E4A", command=self.choose_apk_file).pack(side="right", padx=10)

        # Active Scan Mode Indicator Banner
        mode_banner = ctk.CTkFrame(page, fg_color=COLOR_ACCENT_SURFACE, corner_radius=8,
                                    border_width=1, border_color=COLOR_ACCENT_BORDER)
        mode_banner.pack(fill="x", padx=30, pady=(5, 10))
        self.mode_indicator_lbl = ctk.CTkLabel(mode_banner, text=f"Active Scan Mode: {self.current_scan_mode}",
                                                font=("Arial", 13, "bold"), text_color=COLOR_ACCENT_TEXT)
        ...
        for i, (title, val, sub) in enumerate(cards_data):
            ...
            card = ctk.CTkFrame(cards_frame, fg_color=COLOR_SURFACE, corner_radius=8,
                                 border_width=1, border_color=COLOR_BORDER)
        ...
        # 3. Analysis Results (Middle list - clicking these navigates to their respective tabs)
        ...
        for icon, title, desc, target_page in results_data:
            ClickableRow(list_frame, icon, title, desc,
                         command=lambda tp=target_page: self.show_page(tp)).pack(fill="x", pady=6)

        ClickableRow(list_frame, "✨", "Full AI Analysis", "Overall evaluation and recommendations by AI",
                     command=lambda: self.show_page("Full AI Analysis"), is_ai=True).pack(fill="x", pady=(15, 30))

        return page

    def choose_apk_file(self):
        """Let the user pick a real .apk file; updates the header out of its empty state."""
    ...
    def build_chat_page(self):
        ...
        chat_box = ctk.CTkFrame(page, fg_color=COLOR_SURFACE, corner_radius=12,
                                 border_width=1, border_color=COLOR_BORDER)
        ...
        entry = ctk.CTkEntry(input_frame, placeholder_text="Type your question about the APK here...",
                              fg_color=COLOR_SURFACE_HOVER, border_width=0, height=45, font=("Arial", 13))
        entry.pack(side="left", fill="x", expand=True, padx=(0, 15))

        send_btn = ctk.CTkButton(input_frame, text="Send", width=100, height=45, font=("Arial", 14, "bold"),
                                  fg_color=COLOR_ACCENT, hover_color=COLOR_ACCENT_HOVER)
    ...
    def build_placeholder_page(self, title):
        ...
        back_btn = ctk.CTkButton(header_frame, text="← Back to Dashboard", width=160, height=35,
                                  fg_color=COLOR_SURFACE_HOVER, hover_color="#3D3E4A",
                                  command=lambda: self.show_page("Dashboard"))
        back_btn.pack(side="right")

        content_box = ctk.CTkFrame(page, fg_color=COLOR_SURFACE, corner_radius=12,
                                    border_width=1, border_color=COLOR_BORDER)
        content_box.grid(row=1, column=0, sticky="nsew", padx=30, pady=(0, 30))

        ctk.CTkLabel(content_box, text=f"No data yet — run an analysis to populate '{title}'.",
                     font=("Arial", 15), text_color=COLOR_TEXT_MUTED).pack(expand=True)

        return page

    def show_page(self, page_name):
        """Manage main view navigation (Dashboard, AI Assistant, API Settings, and Result Tabs)"""
